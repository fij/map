                    ...
                    # IF the download was successful,
                        ...
                        for response in responseList:
                            ...
                            for url_target_loop_var in re.findall(r'href=(\".+?\"|\'.+?\')', response.text.encode('utf-8'), flags=re.IGNORECASE):
                                ...
                                url_target = re.sub(r'^.*?\?.*?$', '', url_target)
                                # discard common image formats, and also js, json, xml, cfm, pdf, css, gz, zip
                                url_target = re.sub(r'^.*?\.(jpg|gif|png|jpeg|tiff?|js|json|xml|cfm|pdf|css|gz|zip)$', '', url_target, flags=re.IGNORECASE)
